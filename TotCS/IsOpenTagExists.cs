...
namespace TotCS
{
    ...
    {
        public static async Task<(bool result, long position)> IsOpenTagExists(string filename, string name, long position = 0, Encoding? encoding = null, int streamCount = DefaultStreamCount)
        {
            try
            {
        ...
        private static async Task<(bool result, long position)> ProcessIsOpenTagExists(string filename, string name, long position = 0, Encoding? encoding = null, int streamCount = DefaultStreamCount)
        {
                ...
                try
                {
                    ...
                    while (stream.Position < stream.Length)
                    {
                        ...
                        while (processingChunk.Length > 0)
                        {
                            ...
                            if (index > -1)
                            {
                                long chunkLeft = fileEncoding.GetByteCount(processingChunk[index..]);
